...
public class BlockUtils {
    ...
    private static List<Block> getBlocksUpDown(Location base, int changeX, int changeY, int changeZ, int depthY) {
        ...
        for (int i = 0; i < Math.abs(depthY); ++i) {
            if (i != 0) baseBlock = baseBlock.getLocation().add(0.0, depthPos ? 1.0 : -1.0, 0.0).getBlock();
    ...
    private static List<Block> getBlocksWestEast(Location base, int changeX, int changeY, int changeZ, int depthX) {
        ...
        for (int i = 0; i < Math.abs(depthX); ++i) {
            if (i != 0) baseBlock = baseBlock.getLocation().add(depthPos ? 1.0 : -1.0, 0.0, 0.0).getBlock();
            blocks.addAll(getBlocks(baseBlock.getLocation(), changeX, changeY, changeZ));
        }
        return blocks;
    }

    private static List<Block> getBlocksNorthSouth(Location base, int changeX, int changeY, int depthZ) {
        List<Block> blocks = new ArrayList<>();
        boolean depthPos = depthZ >= 0;
        Block baseBlock = base.getBlock();
        for (int i = 0; i < Math.abs(depthZ); ++i) {
            if (i != 0) baseBlock = baseBlock.getLocation().add(0.0, 0.0, depthPos ? 1.0 : -1.0).getBlock();
            blocks.addAll(getBlocks(baseBlock.getLocation(), changeX, changeY, 0));
        }
        return blocks;
    }
    ...
    public static Set<Block> getSquare(Block b, BlockFace face, int level) {
        ...
        switch (face) {
            ...
            case NORTH: {
                blocks.addAll(getBlocksNorthSouth(b.getLocation(), radius, radius, depth));
                break;
            }
            case SOUTH: {
                blocks.addAll(getBlocksNorthSouth(b.getLocation(), radius, radius, -depth));
    ...
    private static int getDepthByLevel(int level) {
        int depth = 0;
        if (level == 1 || level == 2) {
            if (Math.random() >= level * 0.33) return 0;
            depth = 1;
        }
        if (level == 3) depth = 1;

        if (level == 4 || level == 5) {
            if (Math.random() < (level - 3) * 0.33) {
                depth = 2;
            } else {
                depth = 1;
            }
